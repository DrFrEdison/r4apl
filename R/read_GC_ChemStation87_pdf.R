# function to read ChemStation Version 87 pdf reports
# Currently, this is only tested for GC ChemStation B.04.03-SP1 (87) 2001-2012
# type pattern are necessary to remove the column "Type" from the report pdf
read.GC.ChemStation87.pdf <- function(pdf.file
  ...
  for(i in seq_along(dat$trs$sample.page.no.dub)){
    ...
    dat$final$unit[[ i ]][ which( colnames(dat$final$df[[ i ]]) %in% "Amount")] <- dat$final$column.units[[ i ]][[ 1 ]][ grep("\\% \\(", dat$final$column.units[[ i ]][[ 1 ]])]
