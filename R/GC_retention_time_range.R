...
gc_clean_data <- function(GC_MS, time_range = list(GC_MS = 0.2, GC_TCD1A = 0.25, GC_TCD2B = 0.25)) {
  ...
  if(!is.null(GC_MS$raw$GC_MS)){
    ...
    remove_ms <- which(unlist(lapply(GC_MS$raw$GC_MS, function(x) {
      round(diff(range(x$RT / 60)), 2) < time_range$GC_MS
    })))
  ...
  # Check and remove invalid SIM entries
  if(!is.null(GC_MS$raw$SIM)){
  ...
  if(!is.null(GC_MS$raw$GC_TCD1A)){
    ...
    remove_TCD1A <- which(unlist(lapply(GC_MS$raw$GC_TCD1A, function(x) {
      round(diff(range(x$RT / 60)), 2) < time_range$GC_TCD1A
    })))
  ...
  if(!is.null(GC_MS$raw$GC_TCD2B)){
    ...
    remove_TCD2B <- which(unlist(lapply(GC_MS$raw$GC_TCD2B, function(x) {
      round(diff(range(x$RT / 60)), 2) < time_range$GC_TCD2B
    })))
