...
plot_tcd <- function(tcd.dat){
  ...
  par(mar = marp)
  plot(tcd.dat$time, tcd.dat$TCD1A
       , type = "l"
       , col = r4apl$col[2]
       , ylim = c(0, rangexy(tcd.dat$TCD1A, 20)[2])
       , xlab = "", ylab = "", axes = F)
  ...
  par(new = T)
  plot(tcd.dat$time, tcd.dat$TCD2B, type = "l", ylim = c(0, rangexy(tcd.dat$TCD2B, 20)[2])
       , axes = F
       , col = r4apl$col[5]
       , xlab = "", ylab = "")
  axis(1, cex.axis = cex.axisp)
  ...
  mtext("TCD1A", 2, linep.y, col = r4apl$col[ 2 ])
  mtext("TCD2B", 4, linep.x, col = r4apl$col[ 5 ])
  mtext("TCD", 3, linep.main)
  
  legend("topright", c("TCD1A", "TCD2B"), col = r4apl$col[c(2,5)], lty = 1, cex = cex.axisp)
}
