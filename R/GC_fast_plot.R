fastplot_gc <- function(ID
  ...
  if(!is.null(dir)) db <- db[ db$dir == dir, ]
  if(!is.null(date)) db <- db[ which(as.Date(db$datetime) == date), ]
  if(!is.null(method)) db <- db[ db$method == method, ]
  
  if(length( which(db$data_file == ID) )) warning("Duplicated ID, date | method | dir needed, first match is plotted")
